  ...
  */
  ...
  this.find = function (coll,opts,callback){
    ...
    self.db.collection( coll ).count( selector, function(err,totalDocs){
      ...
      if ( expire_cursor )
        setTimeout( function(){self._clear_cursor(skinCur.cName)}, 1 * 5 * 1000 );
  ...
  this._clear_cursor = function(cName){
    ...
    else l("cursor: " + cName + " already cleared");
  }
  ...
  this._get_batch = function( sc, callback ){
    ...
    sc.nextObject( function( err, doc ){
      ...
      if ( sc.docs_this_batch.length === sc.batch_size || sc.cursor_pos == sc.totalDocsThisQuery){
        l("got last item for this cursor batch", sc.batch_size, sc.cursor_pos, sc.totalDocsThisQuery, sc.docs_this_batch.length);

        //send back copies of the values
        callback( 
          null, 
          [].concat( sc.docs_this_batch ), 
          sc.cName.toString(),
          sc.totalDocsThisQuery*1
        );
        //now clear the cursor
        self._clear_cursor(sc.cName);
